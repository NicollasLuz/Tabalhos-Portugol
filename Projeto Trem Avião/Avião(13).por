programa {	
  ...
  inteiro opcao, fusoOrigem = 0, fusoDestino = 0, quantidadeDePontos, horaDecolagem = 0, minutoDecolagem = 0, horarioChegadaUTC, horarioChegadaLocal, tempoTotalVoo = 0, jaPassouPor = 0
  ...
  */
  ...
  funcao vizualizarDados(){
    ...
    para(inteiro i = 0; i < quantidadeDePontos; i++) {
      formatacaoTempoVooTrecho("Trecho ", nomePontos[i], ": ", calcularETO(distanciaPontos[i], velocidadeCruzeiro))
    ...
    cadeia tempoTotalVooFormatado = formatarHoraMinutos(tempoTotalVoo) 

    //Chama a função tempo total de voo onde é analizado se é horas ou minutos
    formatacaoTempoVoo("Tempo total de voo:  ", tempoTotalVooFormatado)

    //Exibe a hora de chegada em horario mundial
    escreva("Horário de chegada em UTC: ", formatarHoraMinutos(horarioChegadaUTC), "\n")

    //Exibe a hora de chegada em horario local 
  ...
  funcao formatacaoTempoVooTrecho(cadeia texto, cadeia i, cadeia doisPontos, real tv){
    ...
    se(resultado <= 59){
      ...
      se(resultado - resultado2 >= 0.1 ou resultado - resultado2 <= 0.4){
      	
        //Retorna o texto pela função com o texto o número do trecho, separação por 2 pontos, o tempo, mais minutos
      	escreva(texto, i, doisPontos, mat.arredondar(resultado, 0) + 1, " minutos\n")   
      } senao{

      //Exibe o tempo total do voo em minutos
      escreva(texto, i, doisPontos, tv, " minutos\n")
      }
    } senao{

      //Transforma tv para minutos e para inteiro para ir para formatação
      inteiro tv2 = tv * 60

      //Caso não seja em minutos é feita a converção para horas
      escreva(texto, formatarHoraMinutos(tv2), " horas\n")
    }
  }
